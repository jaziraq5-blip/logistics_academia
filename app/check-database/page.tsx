  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>🔍 Check Database Structure</CardTitle>
            <CardDescription>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      <strong>Missing Columns:</strong> {dbInfo.missingColumns?.length || 0}
                    </div>
                    <div>
                      <strong>Wrong Columns:</strong> {dbInfo.wrongColumns?.length || 0}
                    </div>
                  </div>
                </div>

                {dbInfo.missingColumns && dbInfo.missingColumns.length > 0 && (
                  <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                    <h3 className="font-semibold text-red-800 mb-2">❌ Missing Columns:</h3>
                    <div className="flex flex-wrap gap-2">
                      {dbInfo.missingColumns.map((col: string, index: number) => (
                ...
                {dbInfo.wrongColumns && dbInfo.wrongColumns.length > 0 && (
                  <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <h3 className="font-semibold text-yellow-800 mb-2">⚠️ Wrong Columns:</h3>
                    <div className="flex flex-wrap gap-2">
                      {dbInfo.wrongColumns.map((col: string, index: number) => (
                ...
                {dbInfo.needsFix && (
                  <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    ...
                    <p className="text-yellow-700">
                      The database table structure doesn't match what the code expects. 
                      Click "Fix Database" to resolve this.
                    </p>
                  </div>
                )}
              </div>
